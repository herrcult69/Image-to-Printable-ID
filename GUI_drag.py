...
class Line:
    ...
    def make_line(self, canvas):
        line = canvas.create_line(
            self.x1, self.y1, self.x2, self.y2, fill="black", width=2
        )
        return line
...
class GUI:
    def __init__(self, root, img):
        ...
        self.quadrilateral_corners = self.quadrilateral(
            self.w / 2, self.h / 2, self.canvas
        )
        print(self.quadrilateral_corners)
        self.quadrilateral_shape = self.draw_line(
            self.quadrilateral_corners["names"], self.canvas
        )
        ...
        self.coords_label = Label(
            ...
            width=int(self.w / 2),
            height=self.h,
            text="Press anywhere for Coordinates",
        )
    ...
    def OnPress(self, event):
        ...
        for i, point in enumerate(
            self.quadrilateral_corners["names"]
        ):  # Iterate through points
            distance = (
                (point.x - event.x) ** 2 + (point.y - event.y) ** 2
            ) ** 0.5  # Calculate distance
